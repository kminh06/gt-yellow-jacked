...
interface WorkoutState {
  ...
  hydrateFromWorkout: (workout: Workout) => void
  resetWorkout: () => void
...
export const useWorkoutStore = create<WorkoutState>((set) => ({
  ...
  hydrateFromWorkout: (workout) =>
    set({
      ...
      startedAt: new Date(workout.createdAt),
      exercises: workout.exercises.map((exercise) =>
        makeWorkoutExercise(exercise),
      ),
    }),
  ...
  addExercise: (exercise) =>
    set((state) => ({
      ...
      exercises: [...state.exercises, makeWorkoutExercise(exercise)],
    })),
    ...
    set((state) => ({
      exercises: state.exercises.map((ex) =>
        ex.instanceId === instanceId
          ? { ...ex, sets: [...ex.sets, makeDefaultSet()] }
          : ex,
      ),
    })),

  removeSet: (instanceId, setId) =>
    set((state) => ({
      exercises: state.exercises.map((ex) => {
  ...
  updateSet: (instanceId, setId, field, value) =>
    set((state) => ({
      exercises: state.exercises.map((ex) => {
        if (ex.instanceId != instanceId) return ex
        return {
  ...
  toggleSetComplete: (instanceId, setId) =>
    set((state) => ({
      exercises: state.exercises.map((ex) => {
        if (ex.instanceId !== instanceId) return ex
        return {
